...
// Keys are `report-{userId}-{YYYY-MM-DD}.pdf`; show the date, not the raw key.
const reportTitle = (key: string) => {
	const match = key.match(/report-\d+-(\d{4}-\d{2}-\d{2})\.pdf$/);
	return match ? `Account report · ${match[1]}` : key;
};
	...
	return (
		<main>
			<section className="hero">
				...
				<h1>Reports</h1>
				<p className="intro">
					Your account's daily reports, generated by Browser Rendering and stored in R2.
				</p>
				...
				{state.status === "loaded" &&
					state.reports.map((report) => (
						<div className="result" key={report.key}>
							...
							<div style={{ flex: 1 }}>
								<strong>{reportTitle(report.key)}</strong>
								<time>
